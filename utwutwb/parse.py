...
class Converter:
    ...
    def __init__(self) -> None:
        self.converters: Dict[
            Type[exp.Expression], Callable[[exp.Expression], Condition]
        ] = {
    ...
    def _convert_in(self, expression: exp.Expression) -> cond.In:
        ...
        if field:
            return cond.In(
                left=self.convert(expression.this), right=Attribute(name=field.name)
            )
...
class Parser:
    ...
    def __init__(
        self, dialect: Optional[Dialect] = None, converter: Optional[Converter] = None
    ):
